...
use JeffreyVanHees\OnlinePaymentPlatform\Requests\Merchants\UpdateMerchantProfileRequest;
...
class ProfilesResource extends BaseResource
{
     ...
     *
     * @param  CreateMerchantProfileData|array  $data  Profile data including name, configuration, etc.
     ...
     */
    public function create(CreateMerchantProfileData|array $data): Response
    {
     ...
     *
     * @param  string  $profileUid  The unique identifier of the profile
     * @param  UpdateMerchantProfileData|array  $data  Profile update data
     * @return Response API response containing the updated profile data
     *
     * @throws \JeffreyVanHees\OnlinePaymentPlatform\Exceptions\ValidationException When update data is invalid
     * @throws \JeffreyVanHees\OnlinePaymentPlatform\Exceptions\ApiException When profile is not found or other API errors
     * @throws \JeffreyVanHees\OnlinePaymentPlatform\Exceptions\AuthenticationException When API key is invalid
     */
    public function update(string $profileUid, UpdateMerchantProfileData|array $data): Response
    {
        return $this->connector->send(new UpdateMerchantProfileRequest($this->merchantUid, $profileUid, $data));
    }
     ...
     *
     * @throws \JeffreyVanHees\OnlinePaymentPlatform\Exceptions\ApiException When profile is not found, cannot be deleted, or other API errors
     * @throws \JeffreyVanHees\OnlinePaymentPlatform\Exceptions\AuthenticationException When API key is invalid
     */
    public function delete(string $profileUid): Response
    {
        return $this->connector->send(new DeleteMerchantProfileRequest($this->merchantUid, $profileUid));
    }
}
